  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Active Modules</p>
        <div className="grid grid-cols-2 gap-3">
          {active.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}
              ...
              <Link href={`/trackers/${t.id}`}>
                <div className="rounded-2xl p-4 cursor-pointer transition-all hover:border-[var(--text-faint)] h-full"
                  style={{ background: "var(--panel)", border: "1px solid var(--border-s)" }}>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                      style={{ background: "var(--border-light)", color: "#6366f1" }}>
                      {ICONS[t.icon] || ICONS.default}
                    </div>
                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider"
                      style={{ background: "var(--panel-hover)", color: "var(--text-dim)" }}>
                      {t.type}
                    </span>
                  </div>
              ...
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                {["gym", "dsa"].includes(t.id) ? (
                  <button onClick={(e) => { e.preventDefault(); toggleTracker(t.id, false); }}
                    className="w-7 h-7 rounded-lg flex items-center justify-center transition-colors bg-[var(--panel-hover)] hover:bg-[var(--border-strong)]"
                    style={{ color: "var(--text-secondary)" }} title="Deactivate">
                    <Check className="w-3.5 h-3.5 text-emerald-500" />
                  </button>
                ) : (
                  <button onClick={(e) => { e.preventDefault(); handleDelete(t.id); }}
                    className="w-7 h-7 rounded-lg flex items-center justify-center transition-colors bg-[var(--panel-hover)] hover:bg-red-500/20"
          ...
          {active.length === 0 && (
            <div className="col-span-2 rounded-2xl p-8 text-center" style={{ background: "var(--panel)", border: "1px solid var(--border-s)" }}>
              <p className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>No active modules</p>
              <p className="text-xs mt-1" style={{ color: "var(--text-faint)" }}>Activate a template below or create your own</p>
            </div>
          )}
        </div>
      </div>
      ...
      {inactive.length > 0 && (
        <div className="space-y-4 pt-4">
          <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Available Templates</p>
              ...
              <div className="flex gap-2">
                {Object.keys(ICONS).filter(k => k !== "default").map(k => (
                  <button key={k} onClick={() => setNewIcon(k)}
                    ...
                    style={{
                      background: newIcon === k ? "#ffffff10" : "var(--panel-hover)",
                      borderColor: newIcon === k ? "#ffffff30" : "var(--border-strong)",
                      color: newIcon === k ? "var(--text-heading)" : "var(--text-tertiary)"
                    }}>
                    {ICONS[k]}
                  </button>
                ))}
